...
class _fasterRCNN(nn.Module):
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes):
        ...
        cls_score = []
        for x in range(50):
            z = self.RCNN_cls_score[x](pooled_feat.cpu())
            cls_score.append(z)
        #cls_score = [self.RCNN_cls_score[x](pooled_feat.cpu()) for x in range(50)]
        tanh_layer = nn.Tanh()
        ...
        if self.training:
            ...
            mse_loss = torch.mean(mse_loss_fun(cls_score, rois_vector), dim=1)
        ...
        if self.training:
            return rois, cls_prob, bbox_pred, rpn_loss_cls.unsqueeze_(0), rpn_loss_bbox.unsqueeze_(0), RCNN_loss_cls.unsqueeze_(0), RCNN_loss_bbox.unsqueeze_(0), rois_label
        else:
    ...
    def _init_weights(self):
        ...
        for x in self.RCNN_cls_score:
            normal_init(x, 0, 0.01, cfg.TRAIN.TRUNCATED)
