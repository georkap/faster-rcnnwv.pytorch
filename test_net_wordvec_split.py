...
if __name__ == '__main__':
  ...
  # ship to cuda
  if args.cuda:
    im_data = im_data.cuda(device=args.gpus[0])
    im_info = im_info.cuda(device=args.gpus[0])
    num_boxes = num_boxes.cuda(device=args.gpus[0])
    gt_boxes = gt_boxes.cuda(device=args.gpus[0])
  ...
  if args.cuda:
    fasterRCNN.cuda(device=args.gpus[0])
  ...
  for i in range(num_images):
      ...
      if cfg.TEST.BBOX_REG:
          ...
          # Optionally normalize targets by a precomputed mean and stdev
            if args.class_agnostic:
                box_deltas = box_deltas.view(-1, 4) * torch.FloatTensor(cfg.TRAIN.BBOX_NORMALIZE_STDS).cuda(device=args.gpus[0]) \
                           + torch.FloatTensor(cfg.TRAIN.BBOX_NORMALIZE_MEANS).cuda(device=args.gpus[0])
      ...
      for ii in range(scores.shape[0]): # for all 300 the bounding boxes of an image
          ...
          for k in range(1, 21): # we calculate the dot product with each possible class
              res_vector[ii].append(cosine_similarity(scores.cpu().numpy()[ii], np.array(name_vectors[k])))
          
      scores = softmax(torch.tensor(res_vector).cuda(device=args.gpus[0]))
